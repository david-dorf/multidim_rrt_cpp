...
public:
  RRT3DNode()
      : Node("RRT3DNode"), start_x(0), start_y(0), start_z(0), goal_x(5), goal_y(5), goal_z(5),
        step_size(0.5), goal_tolerance(0.5), map_sub_mode(false), obstacle_sub_mode(false),
...
private:
  ...
  float get_dist(const std::vector<float> &vec1, const std::vector<float> &vec2)
  {
    float sum = 0;
                            ...
                            std_srvs::srv::Empty::Response::SharedPtr)
  {
    ...
    while (node_list.size() < node_limit)
    {
      ...
      float min_goal_distance = INFINITY;
      float min_node_distance = INFINITY;
      std::vector<float> min_node_vec;
      TreeNode *nearest_node;
      for (TreeNode *node : node_list)
      {
        const float goal_distance = get_dist(node->val, goal_position);
        ...
        std::vector<float> node_vec = {node->val[0] - rand_x,
                                       node->val[1] - rand_y,
                                       node->val[2] - rand_z};
        const float node_distance = get_dist(node_vec, {0, 0, 0});
      ...
      const float new_node_x = nearest_node->val[0] -
                               (min_node_vec[0] / min_node_distance) * step_size;
      const float new_node_y = nearest_node->val[1] -
                               (min_node_vec[1] / min_node_distance) * step_size;
      const float new_node_z = nearest_node->val[2] -
                               (min_node_vec[2] / min_node_distance) * step_size;
